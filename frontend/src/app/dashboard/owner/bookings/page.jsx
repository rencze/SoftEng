...
export default function BookingManagementPage() {
  ...
  const handleAccept = async (booking) => {
    try {
      await axios.patch(`${API_URL}/${booking.bookingId}/status`, { statusId: 2, remarks: "Accepted" });
      fetchBookings();
    } catch (err) {
      console.error(err);
    }
  ...
  const handleCancel = async (booking) => {
    ...
    try {
      await axios.patch(`${API_URL}/${booking.bookingId}/status`, { statusId: 3, remarks: "Cancelled" });
      fetchBookings();
    } catch (err) {
      console.error(err);
    }
  ...
  const handleReschedule = async () => {
    try {
      await axios.put(`${API_URL}/${selectedBooking.bookingId}`, { date: newDate, time: newTime });
      fetchBookings();
      closeModal();
    } catch (err) {
      console.error(err);
        ...
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="px-6 py-3 text-left font-medium text-gray-600 uppercase">Technician</th>
            ...
            {bookings.length > 0 ? (
              bookings.map(b => (
                <tr key={b.bookingId} className="hover:bg-gray-50 transition">
                  <td className="px-6 py-4 font-semibold text-gray-800">{b.customerName}</td>
                  ...
                  <td className="px-6 py-4 text-gray-600">{b.time || `${b.startTime} - ${b.endTime}`}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-sm font-medium ${
                      b.statusName === "Incoming" ? "bg-indigo-100 text-indigo-700" :
                      b.statusName === "Reschedule" ? "bg-yellow-100 text-yellow-700" :
