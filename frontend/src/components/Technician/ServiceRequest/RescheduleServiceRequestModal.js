...
export default function ServiceRescheduleModal({ 
  isOpen, 
  onClose, 
  serviceRequestId,
  onRescheduleSuccess 
  ...
  useEffect(() => {
    const fetchBookingDetails = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:3001/api/service-request-bookings/${serviceRequestId}`, {
  ...
  const handleReschedule = async () => {
    ...
    if (!serviceRequestId) {
      console.error("❌ serviceRequestId is undefined!");
      alert("Error: Service Request ID is missing. Please try again.");
      return;
    }
    ...
    const payload = {
      timeSlotId: Number(selectedHour.id),
    ...
    try {
      const res = await fetch(`http://localhost:3001/api/service-request-bookings/reschedule/${serviceRequestId}`, {
        method: "PUT",
        headers: {
      ...
      if (res.ok) {
        ...
        if (onRescheduleSuccess) {
          onRescheduleSuccess(); // This will refresh the parent and close modal
        } else {
          // Fallback
          router.refresh();
          onClose();
      ...
      alert("Error rescheduling booking: " + err.message);
    }
  };
                    ...
                    <div className="text-sm text-gray-600 space-y-3">
                      ...
                      {bookingDetails && (
                        <div className="mb-3 pb-3 border-b border-gray-200">
                          ...
                          <p className="mb-1"><span className="font-medium">Date:</span> {moment(bookingDetails.requestedDate).format("MMMM D, YYYY")}</p>
                          ...
                          {bookingDetails.assignedTechnician && (
                            <p><span className="font-medium">Technician:</span> {bookingDetails.assignedTechnician}</p>
                          )}
                        </div>
                      )}
