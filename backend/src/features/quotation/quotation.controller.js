...
async function createQuotationController(req, res) {
  try {
    ...
    console.log('Quotation created successfully:', {
      ...
      serviceRequestId: newQuotation.serviceRequestId,
...
// Check expired quotations
async function checkExpiredQuotationsController(req, res) {
  try {
...
async function getQuotationExpiryInfoController(req, res) {
  try {
    const expiryInfo = await getQuotationExpiryInfo(req.params.id);
    if (!expiryInfo) return res.status(404).json({ error: "Quotation not found" });
    res.json(expiryInfo);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
